//汉化杂项
var cnItems = {
    'Worms': '蠕虫',
    'Minnows': '小鱼',
    'Guppies': '孔雀鱼',
    'Insects': '昆虫',
    'Mussels': '青口贝',
    'Crustaceans': '甲壳动物',
    'Squid': '乌贼',
    'Ground Fish': '底栖鱼',
    'Fly Tackle': '飞铲',
    'Sturgeon': '鲟鱼',
    'Chub': '鲢鱼',
    'Bass': '鲈鱼',
    'Spinner Lure': '旋转诱饵',
    'Harpoon': '鱼叉',
    'River': '河流',
    'Lake': '湖泊',
    'Shop': '商店',
    'Forage for worms': '寻找蠕虫',
    'Cast out line': '抛出线',
    'Reel in line': '收回线',
    'Deep Sea': '深海',
    'Spoon Lure': '匙形鱼饵',
    'Spear Fishing': '鱼叉捕鱼',
    'Fuel': '燃料',
    'Reef': '暗礁',
    'Pier': '码头',
    'Cages': '笼子',
    'Bobber': '浮子',
    'Hire a worker to be assigned to an available area': '雇佣一名工人分配到一个可用的区域',
    'Send out "influencers" to spread false rumors about specific lakes to free them for your workers': '派出“影响者”散布关于特定湖泊的虚假谣言，为你的工人释放它们',
    'Tackle': '钓具',
    'Bait': '鱼饵',
    'Misc': '杂项',
    'Salmon': '鲑鱼',
    'Hull': '船壳',
    'Cabin': '舱室',
    'Engine': '发动机',
    'Anchor': '锚',
    'Crawdads': '小龙虾',
    'Trout': '鳟鱼',
    'Pike': '梭鲈',
    'Redfish': '红鱼',
    'Snapper': '鲷鱼',
    'Crabs': '螃蟹',
    'Mackerel': '马鲛鱼',
    'Settings': '设置',
    'Hogfish': '猪鲨',
    'Lobster': '龙虾',
    'Grouper': '石斑鱼',
    'Tuna': '金枪鱼',
    'Swordfish': '剑鱼',
    'Barracuda': '梭子鱼',
    'Moray Eel': '海鳗',
    'Lemon Shark': '柠檬鲨',
    'Tiger Shark': '虎鲨',
    'Black Drum': '石首鱼',
    'Tarpon': '大海鲢',
    'Total fish caught': '总捕鱼量',
    'Whitefish': '白鲑',
    'Marlin': '大马林鱼',
    'Stonefish': '石鱼',
    'Lingcod': '鳕鱼',
    'Mako Shark': '灰鲭鲨',
    'Thresher Shark': '长尾鲨',
    'Workers': '工人',
    'Argentina abolishes ranch dressing, this story will shake you!': '阿根廷废除牧场调料，这个故事会动摇你！',
    'Poland cancels paperclip factories, could they be hiding something?': '波兰取消了回形针工厂，他们可能藏匿什么东西吗？',
    'Costa Rica starts debates about toilet paper, experts are perplexed.': '哥斯达黎加开始讨论卫生纸，专家们感到困惑。',
    'Israel de-materializes peacocks, and the results are spectacular!': '以色列将孔雀变为物质化，结果非常壮观！',
    'Sweden begins to recall napkins, more at 6.': '瑞典开始召回餐巾纸，更在6。',
    'Local Group Control': '本地集团控制',
    'Expand the workforce': '扩大劳动力',
    '>>': '>>',
    '>': '>',
    '<<': '<<',
    '<': '<',
    'Hire a worker to be assigned to an available area': '雇佣一名工人分配到一个可用区域',
    'Mass Angler Dissuasion': '大众垂钓者劝阻',
    'Buy Adjoining Lot': '购买相邻的地段',
    'Unlock Opportunities': '解锁机会',
    'Japan': '日本',
    'Thailand': '泰国',
    'Construct fences and warning signs along river banks to deter competition': '沿河岸建设围栏和警示标志，以阻止竞争',
    'Your current building is getting cramped and there is a great empty lot next to you for sale': '你现在的建筑物变得狭窄，旁边有一个很棒的空地出售',
    'Hire additional upper management so you have time to focus on exterior issues.': '聘请额外的高层管理人员，以便您有时间专注于外部问题。',
    'Private Pier Allocation': '私人码头分配',
    'Significant purchasing of public and private piers enables greater utilization of your workers': '大量购买公共和私人码头可以提高工人的利用率',
    'Opportunities and Morality': '机遇与道德',
    'Poland': '波兰',
    'Commercial Espionage': '商业间谍活动',
    'Accept without pay': '无偿接受',
    'Decline the offer': '拒绝报价',
    'You run a legitimate and moral establishment and want nothing to do with this man.': '你管理着合法和道德的机构，不想和这个人有任何瓜葛。',
    'You can hear what he has to say without giving anything up.': '你可以在不放弃任何东西的情况下听到他要说的话。',
    'Zander is a former manager at your biggest competitor. For the right price, he can reveal their business plans.': '桑德尔是你最大的竞争对手是前经理。 以合适的价格，他可以透露他们的商业计划。',
    'Tear-Resistant Nets': '抗撕裂网',
    'Purchase Office Space': '购买办公空间',
    "You'll need more space for departments if you want to grow your business": '如果您想拓展业务，则需要更多部门空间',
    'Smaller net meshes mean they can be reinforced to combat sharp coral': '较小的网状网意味着它们可以加强以对抗尖锐的珊瑚',
    'Open a New Location': '打开一个新位置',
    'The other side of town is rapidly developing and no fishing shops -- best to take advantage quickly': '城镇的另一边正在迅速发展，没有渔场 - 最好快速利用',
    'Vietnam': '越南',
    'Malaysia': '马来西亚',
    'Spear Wielding SCUBA Teams': '矛挥舞潜水队',
    'Groups of men and women given equipment to act like fish and hunt them down': '一群男人和女人给予设备像鱼一样捕杀它们',
    'Express your interest': '表达你的兴趣',
    'Send a takedown notice': '发送删除通知',
    'Video Game Avenues': '视频游戏途径',
    "A software developer, Camille, is inspired by your business's success and she has some grand plans for an online fishing game.": '软件开发人员Camille的灵感来自于您的业务成功，她有一些在线钓鱼游戏的宏伟计划。',
    'Camille has no right to be using your business and intellectual property in her game without your permission.': '未经您的许可，Camille无权在游戏中使用您的商业和知识产权。',
    "You don't know much about video games but she seems enthusiastic.": '你对电子游戏知之甚少，但她似乎很热情。',
    'Ask for proof': '索要证明',
    "You can't be firing people on a whim without any proof.": '没有任何证据，你不可能随心所欲地解雇人。',
    'Workplace Misconduct': '工作场所不端行为',
    'A trusted employee, Diana, storms into your office demanding her co-worker be fired after she was slapped.': '一位值得信赖的员工戴安娜闯入你的办公室，要求她的同事在被打耳光后被解雇。',
    'Question her allegations': '对她的指控提出质疑',
    "You've known who she is talking about for many years. Something doesn't add up.": '你已经知道她多年来一直在谈论谁。 有些东西没有加起来。',
    'Deep Sea Barges': '深海驳船',
    'Fleets of large ships to harvest the darkest corners of every ocean': '大型舰队的船队收获每个海洋最黑暗的角落',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',

};

function cnItem(text) {
    //数组里面有的，返回中文
    for (var i in cnItems) {
        if (text == i) {
            return cnItems[i];
        }
    }
    //数组里面没有的，原样返回
    for (var i in cnItems) {
        if (text != i) {
//            console.log("需汉化的英文Item：" + text);
            return text;
        }
    }
}



//汉化标题
var cntit = {
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    '': '',
    

};

function cntitle(text) {
    //数组里面有的，返回中文
    for (var i in cntit) {
        if (text == i) {
            return cntit[i];
        }
    }
    //数组里面没有的，原样返回
    for (var i in cntit) {
        if (text != i) {
            console.log("需汉化的英文标题：" + text);
            return text;
        }
    }
}

